...
def cal_topn(q_feats,q_ids,d_feats,d_ids, cfg, max_rank=100):
    ...
    indices = np.argsort(-sim,axis=1) #将矩阵a按照gallery排序，并返回排序后的下标
    ...
    if 1:
        for i in range(num_q):
            ...
            #save_img
            #if sorted_simi[0][1][0]-sorted_simi[1][1][0]>=0.2:
            #   continue
            #if sorted_simi[0][0]==q_id:
            #    continue

            q_img = query_data[i].copy()
            txt_img=np.ones((28,cfg.input_shape[2],3))*255
            q_img = np.vstack([q_img,txt_img])
            cv2.putText(q_img, str(q_id),(0,cfg.input_shape[1]+13), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
            #cv2.imwrite('./demo_imgs/'+'ori.jpg',q_img)

            qd_img = distractor_data[sim_temp[q_id][1]].copy()
            text1 = str(q_id)
            #只查看不是统一目标的数据
            if text1==str(sorted_simi[0][0]):
                continue
            text2 = str(sorted_simi.index((q_id,sim_temp[q_id]))+1)+": "+str(round(sim_temp[q_id][0],3))
            txt_img=np.ones((28,cfg.input_shape[2],3))*255
            qd_img = np.vstack([qd_img,txt_img])
            cv2.putText(qd_img, text1, (0,cfg.input_shape[1]+13), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
            cv2.putText(qd_img, text2, (2,cfg.input_shape[1]+26), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)

            demo_imgs_h=[q_img,qd_img]
            demo_imgs = []
            flag = True
            for ii in range(5):
                if ii==0:
                    ...
                    jj_range_start = (ii-1)*10+8
                    jj_range_end = ii*10+8
                for jj in range(jj_range_start, jj_range_end):
                    d_img = distractor_data[sorted_simi[jj][1][1]].copy()
                    text1 = str(sorted_simi[jj][0])
                    text2 = str(jj+1)+": "+str(round(sorted_simi[jj][1][0],3))
                    txt_img=np.ones((28,cfg.input_shape[2],3))*255
                    d_img = np.vstack([d_img,txt_img])
                    cv2.putText(d_img, text1, (0,cfg.input_shape[1]+13), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
                    cv2.putText(d_img, text2, (2,cfg.input_shape[1]+26), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
            ...
            cv2.imwrite(cfg.save_demo_imgs+str(i)+'_'+str(sorted_simi[0][0])+'_'+str(text1)+'.jpg',final_demo_imgs)
...
if cfg.dataset=="carback" or cfg.dataset=="carfront":
    ...
    print("has_plate")
    cal_topn(  ,query_id_hasplate,distractor_feature,distractor_id)
    print("no_plate")
